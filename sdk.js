...
SDK.play = function(file, channelaudio) {
	...
	if (SDK.canPlayAudio == null) {
		...
		/**
		 * This allows the audio to be initialized inside a button callback.
		 * This is require to auto play on mobile Chrome and Safari.
		 */
		setTimeout(function() {
			if (SDK.canPlayAudio == false) {
				...
				var html = "<div id='sdkplaybutton' style='position:fixed;bottom:32px;left:32px;z-index:164;'><img onclick='SDK.initAudio()' width='64' src='"
...
function WebLiveChatListener() {
		...
		html = html
			...
			+ "<div id='" + this.prefix + "box' class='" + this.prefix + "box' " + backgroundstyle + ">"
				+ "<div class='" + this.prefix + "boxmenu'>"
					...
					+ "<span style='float:right'><a id='" + this.prefix + "boxmin' class='" + this.prefix + "boxmin' onclick='return false;' href='#'><img src='" + SDK.url + "/images/icon_minimize.png'></a>";
					if (this.showBoxmax) {
		...
		if (this.promptContactInfo) {
			html = html
				+ "<div id='" + this.prefix + "contactinfo' class='" + this.prefix + "box' " + backgroundstyle + ">"
					+ "<div class='" + this.prefix + "boxmenu'>"
						+ "<span style='float:right'><a id='" + this.prefix + "contactboxmin' class='" + this.prefix + "contactboxmin' onclick='return false;' href='#'><img src='" + SDK.url + "/images/icon_minimize.png'></a>"
					+ "</div>\n"
		...
		if (this.promptEmailChatLog) {
			html = html
				+ "<div id='" + this.prefix + "emailchatlogdialog' class='" + this.prefix + "box' " + backgroundstyle + ">"
					+ "<div class='" + this.prefix + "boxmenu'>"
						+ "<span style='float:right'><a id='" + this.prefix + "emailchatlogdialogmin' class='" + this.prefix + "boxmin' onclick='return false;' href='#'><img src='" + SDK.url + "/images/icon_minimize.png'></a>"
					+ "</div>\n"
	...
	this.humanAgent = function() {
		if (this.connection != null) {
			this.connection.humanAgent();
			document.getElementById(this.prefix + 'chat').value = '';
		}
		return false;
	};
...
SDK.updateAvatar = function(responseMessage, speak, urlprefix, elementPrefix, channelaudio, afterFunction, nativeVoice, lang, voice) {
	...
	var useVideo = video != null && SDK.useVideo != false && (SDK.useVideo == true || !(SDK.isSafari() && SDK.isIPhone()));
	if (isVideo && useVideo) {
		...
		html = html
			+ "<div id='" + this.prefix + "avatar-div' style='" + hideAvatar + "'>"
				...
				+ "<div id='" + this.prefix + "avatar-video-div' style='display:none;text-align:center;background-position:center;" + divHeight + divWidth + background + "background-repeat: no-repeat;'>"
					+ "<video id='" + this.prefix + "avatar-video' autoplay preload='auto' style='background:transparent;" + minHeight + minAvatarWidth + "'>"
...
function LiveChatConnection() {
	...
	// humanAgent function added by Muhammad Daniyal
	/**
	 * Exit from the current private channel.
	 * This call is asynchronous, any error or success with be sent as a separate message to the listener.
	 */
	this.humanAgent = function() {
		this.checkSocket();
		this.socket.send("__human_agent__");
	};
	// change Ended
	/**
	 * Change to spy mode.
